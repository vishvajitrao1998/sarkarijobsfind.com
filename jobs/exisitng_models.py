...
class JobImportantDate(models.Model):
    job = models.ForeignKey(Job, on_delete=models.CASCADE, related_name='important_dates')
    title = models.CharField(max_length=100)  # e.g. "Application Start"
    date = models.CharField(max_length=50)   # flexible (some dates are "Soon")
...
class JobApplicationFee(models.Model):
    job = models.ForeignKey(Job, on_delete=models.CASCADE, related_name='fees')
    category = models.CharField(max_length=100)  # GEN/OBC/SC/ST
    description = RichTextField(null=True, blank=True)
...
class JobAgeLimit(models.Model):
    job = models.OneToOneField(Job, on_delete=models.CASCADE, related_name='age_limit')
...
class JobSyllabus(models.Model):
    job = models.ForeignKey(Job, on_delete=models.CASCADE, related_name='syllabus')
...
class Guide(models.Model):
    ...
    guides = RichTextField(null=True, blank=True)
